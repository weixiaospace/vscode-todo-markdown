...
- Extension ID：`WeiXiao.todo-md`
- Display name：`Todo (Markdown)`
- Publisher：`WeiXiao`（marketplace 上大小写敏感）
...
前置：publisher `WeiXiao` 已在 https://aka.ms/vscode-create-publisher 注册（用 Microsoft 账号），PAT 已在 https://dev.azure.com 签发（Marketplace → Manage 权限）。

```sh
pnpm exec vsce login WeiXiao       # 首次，输入 PAT
...
- `pnpm exec vsce ls` 可在不打包的情况下列出即将进 vsix 的文件，调整 `.vscodeignore` 后先 `vsce ls` 预览
